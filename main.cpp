...
int main() {
    ...
    my_vector<std::string> test = my_vector<std::string>(5, "string");
    int g = 0;
    for (auto &i : test) {
        i = std::to_string(g);
        g++;
...
//    int pos = 0;
//    std::cin >> pos;
//    std::cout << "element at " << pos << "th position is " << test.at(pos); // этот проверяет oorv [] no check
...
//    }
    ...
    std::string value = "WER";

//
//    std::cin >> temp;
    test.insert(my_iterator(&test[position]), value);
