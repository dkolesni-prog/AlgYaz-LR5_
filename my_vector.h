...
public:
    ...
    const T &operator[](u_int pos) const  {
    ...
    void push_back(const T &value) { //PASS
        if (number_of_elements == capacity_) reserve(capacity_ * factor);
    ...
    my_iterator<T> insert( my_iterator<T> iter, T value) {
        ...
        auto initial_value = *iter.pos;
        my_iterator<T> iter_to_initial = iter;

        for (auto it = ++iter; it <= this->end(); ++it) {

            auto prev = --it;
            auto prev_value = *prev.pos;

            auto cur = ++it;
            auto cur_value = *cur.pos;

            //*cur.pos = prev_value;

        }
        *iter_to_initial.pos = value;


//        auto tmp = *iter.pos;
//        for (auto it = iter++; it <= this->end(); ++it){
//            auto tmp2 = *it.pos; // значение куда смотрит итератор
//            *it.pos = tmp; //
//            tmp = tmp2;
//        }
//        *iter.pos = value;
